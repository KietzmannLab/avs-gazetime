#!/usr/bin/env python3
"""
Plot memorability predictions from MEG decoding analysis.

This script:
1. Loads predicted memorability values from decoder across timepoints
2. Plots correlation between predicted memorability and fixation duration over time
3. Plots duration by memorability quartile at peak prediction timepoint
"""
...
from avs_gazetime.decoding.decoding_params import MEMORABILITY_TARGETS
...
def load_prediction_data(subjects, target_col, ch_type="mag"):
    ...
    for subject in subjects:
        ...
        pred_file = os.path.join(
            plots_dir,
            "memorability_decoder",
            f"predicted_memorability_{target_col}_{subject_str}_{ch_type}.csv"
        )
...
def compute_correlation_over_time(predictions_df):
    """Compute correlation between predicted memorability and duration at each timepoint."""
    results = []
        ...
        for timepoint in timepoints:
            ...
            if len(time_data) > 10:
                corr, pval = stats.pearsonr(
                    time_data['predicted_memorability'],
                    time_data['duration']
                )
                results.append({
                    'subject': subject,
...
def plot_quartile_analysis(predictions_df, corr_df, output_dir, target_col):
    ...
    for subject, peak_time in peak_times.items():
        ...
        subject_data['quartile_mem'] = pd.qcut(
            subject_data['predicted_memorability'],
            NUM_QUARTILES,
            labels=Q_LABELS,
            duplicates='drop'
        )
    ...
    save_path = os.path.join(output_dir, f"duration_per_predicted_mem_quartile_{target_col}.pdf")
    plt.savefig(save_path)
...
def compute_quartile_regression(quartile_df, target_col):
    ...
    quartile_means = quartile_df.groupby(["subject", "quartile_mem"])["duration"].mean().reset_index()
...
def main():
    """Main function to create memorability prediction plots."""
    print("=== MEMORABILITY PREDICTION PLOTTING ===\n")
