...
if __name__ == "__main__":
    ...
    from avs_analysis.tools.avs_directory_tools import get_data_dirs
    _,_,project_dir = get_data_dirs(server="uos", add_project_dir=True)

    out_dir = os.path.join(project_dir, "analysis", "semantic_clusters")
    ...
    # store the csv 
    df.to_csv(os.path.join(out_dir, f"silhouette_scores_per_k_{roi}_{min_clusters}_{max_clusters}_{stepsize}.csv"))
    df_labels_all.to_csv(os.path.join(out_dir, f"cluster_labels_all_{roi}_{min_clusters}_{max_clusters}_{stepsize}.csv"))
